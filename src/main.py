...
async def _process_listings(client, listings: list[Listing]) -> tuple[int, int]:
    """Store new listings. Returns (new_count, skipped_count)."""
...
async def run_once() -> None:
    ...
    async with build_client() as client:
        ...
        for cat_id in KA_CATEGORY_IDS.split(","):
            ...
            if not cat_id:
                continue
            run_id = database.log_run_start("kleinanzeigen", f"cat={cat_id}")
            try:
                listings = await kleinanzeigen.search(
                    client, query=KA_QUERY, location_id=KA_LOCATION_ID,
                    distance=KA_DISTANCE, category_id=cat_id,
                )
                new, skipped = await _process_listings(client, listings)
                database.log_run_end(run_id, len(listings), new, skipped)
            except Exception as e:
                logger.exception("Error fetching Kleinanzeigen (category %s)", cat_id)
                database.log_run_end(run_id, 0, 0, 0, str(e))
        ...
        for re_type in IS24_REAL_ESTATE_TYPES.split(","):
            ...
            if not re_type:
                continue
            run_id = database.log_run_start("immoscout24", re_type)
            try:
                listings = await immoscout.search(
                    client, real_estate_type=re_type,
                    geocoordinates=IS24_GEOCOORDINATES,
                    price_min=IS24_PRICE_MIN, price_max=IS24_PRICE_MAX,
                )
                new, skipped = await _process_listings(client, listings)
                database.log_run_end(run_id, len(listings), new, skipped)
            except Exception as e:
        ...
        for search_def in IW_SEARCHES.split(";"):
            ...
            run_id = database.log_run_start("immowelt", search_def)
            try:
                listings = await immowelt.search(
                    client, distribution_types=dist_types.strip(),
                    estate_types=estate_types.strip(), locations=IW_LOCATIONS,
                )
                new, skipped = await _process_listings(client, listings)
                database.log_run_end(run_id, len(listings), new, skipped)
            except Exception as e:
